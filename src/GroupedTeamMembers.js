...
const GroupedTeamMembers = ({employess, selectedTeam, setTeam }) => {
  ...
  function groupTeamMembers() {
    ...
    var teams = [];
    var teamAMembers = employess.filter((employee) => employee.teamName === 'TeamA')
    var teamA = {team: 'TeamA', members: teamAMembers, collapsed: selectedTeam === 'TeamA' ? false : true}
    teams.push(teamA);

    var teamBMembers = employess.filter((employee) => employee.teamName === 'TeamB')
    var teamB = {team: 'TeamB', members: teamBMembers, collapsed: selectedTeam === 'TeamB' ? false : true}
    teams.push(teamB);

    var teamCMembers = employess.filter((employee) => employee.teamName === 'TeamC')
    var teamC = {team: 'TeamC', members: teamCMembers, collapsed: selectedTeam === 'TeamC' ? false : true}
    teams.push(teamC);

    var teamDMembers = employess.filter((employee) => employee.teamName === 'TeamD')
